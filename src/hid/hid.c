...
const void *mty_hid_device_get_extra_data(struct hid *hid, uint32_t id, MTY_ExtraData type, size_t *size)
{
	struct hid_dev *device = mty_hid_get_device_by_id(hid, id);
	if (!device)
		return NULL;
	...
	switch (hid_driver(device)) {
		case MTY_CTYPE_PS4:
			return ps4_get_extra_data(device, type, size);
		case MTY_CTYPE_PS5:
			return ps5_get_extra_data(device, type, size);
	}
